      ...
      (function(global, factory) {
        ...
        pp$1.isLet = function(context) {
          ...
          if (isIdentifierStart(nextCh, true)) {
            var pos2 = next + 1;
            while (isIdentifierChar(this.input.charCodeAt(pos2), true)) {
              ++pos2;
            }
        ...
        pp$1.checkExport = function(exports4, name2, pos2) {
          if (!exports4) {
            return;
          }
          if (has(exports4, name2)) {
            this.raiseRecoverable(pos2, "Duplicate export '" + name2 + "'");
          }
          exports4[name2] = true;
        };
        ...
        pp$5.declareName = function(name2, bindingType, pos2) {
          ...
          if (redeclared) {
            this.raiseRecoverable(pos2, "Identifier '" + name2 + "' has already been declared");
          }
        };
        ...
        pp$6.startNodeAt = function(pos2, loc) {
        ...
        pp$6.finishNodeAt = function(node, type, pos2, loc) {
        ...
        pp$9.readInt = function(radix, len2) {
          var start = this.pos, total = 0;
          for (var i = 0, e = len2 == null ? Infinity : len2; i < e; ++i) {
        ...
        pp$9.readHexChar = function(len2) {
          var codePos = this.pos;
...
`;
      ...
      class WebGLKernel extends GLKernel {
        ...
        updateMaxTexSize() {
          const { texSize, canvas: canvas2 } = this;
          if (this.maxTexSize === null) {
            let canvasIndex = canvases.indexOf(canvas2);
            if (canvasIndex === -1) {
        ...
        build() {
          ...
          const { texSize, context: gl, canvas: canvas2 } = this;
          gl.enable(gl.SCISSOR_TEST);
          if (this.pipeline && this.precision === "single") {
            gl.viewport(0, 0, this.maxTexSize[0], this.maxTexSize[1]);
            canvas2.width = this.maxTexSize[0];
            canvas2.height = this.maxTexSize[1];
          } else {
            gl.viewport(0, 0, this.maxTexSize[0], this.maxTexSize[1]);
            canvas2.width = this.maxTexSize[0];
            canvas2.height = this.maxTexSize[1];
  ...
  vTexCoord = aTexCoord;
      ...
      class GPU {
        ...
        createKernel(source, settings) {
          ...
          const kernels2 = this.kernels;
          if (this.mode === "dev") {
            const devKernel = gpuMock(source, upgradeDeprecatedCreateKernelSettings(settings));
            kernels2.push(devKernel);
            return devKernel;
          }
        ...
        combineKernels() {
          ...
          if (firstKernel.kernel.constructor.mode === "cpu")
            return combinedKernel;
          const canvas2 = arguments[0].canvas;
          const context = arguments[0].context;
          const max = arguments.length - 1;
          for (let i = 0; i < max; i++) {
            arguments[i].setCanvas(canvas2).setContext(context).setPipeline(true);
          }
          return function() {
...
var gpuBrowserExports = gpuBrowser.exports;
...
function initKernels(gpu2, PARTICLE_COUNT2) {
  ...
  let updateSpeed = gpu2.createKernel(
    function(_pos, _speed, PARTICLE_COUNT3, CANVAS_SIZE2, mouse) {
      ...
      let bottom = pos2[1] > CANVAS_SIZE2 && speed2[1] > 0;
      let top = pos2[1] < 0 && speed2[1] < 0;
      if (right || left) {
        speed2 = [-speed2[0], speed2[1]];
      }
      if (bottom || top) {
        speed2 = [speed2[0], -speed2[1]];
      }
...
function FPS() {
  {
    let curr_time = /* @__PURE__ */ new Date();
